...
import Foreign.Lua.Raw.Call (PreCFunction, hslua_newhaskellfunction)
...
pushHaskellFunction hsFn = do
  errConv <- Lua.errorConversion
  let preCFn = flip (runWithConverter errConv) $ toHaskellFunction hsFn
  ptr <- Lua.liftIO $ newStablePtr preCFn
  liftLua $ \l -> hslua_newhaskellfunction l ptr
